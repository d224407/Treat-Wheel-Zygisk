...
int dl_cb(struct dl_phdr_info *info, size_t size, void *data) {
  ...
  struct elf_img *img = (struct elf_img *)data;
...
bool _find_module_base(struct elf_img *img) {
  dl_iterate_phdr(dl_cb, img);

  return img->base != NULL;
}

size_t calculate_valid_symtabs_amount(struct elf_img *img) {
  size_t count = 0;
...
struct elf_img *elf_create(const char *elf, void *base) {
  struct elf_img *img = (struct elf_img *)calloc(1, sizeof(struct elf_img));
  if (!img) {
    LOGE("Failed to allocate memory for struct elf_img");
    ...
    if (dyn) for (ElfW(Dyn) *d = dyn; d->d_tag != DT_NULL; ++d) {
      ...
      switch (d->d_tag) {
        ...
        case DT_FINI_ARRAYSZ: {
          img->fini_array_count = d->d_un.d_val / sizeof(ElfW(Addr));
...
bool _load_symtabs(struct elf_img *img) {
...
ElfW(Addr) GnuLookup(struct elf_img *restrict img, const char *name, uint32_t hash, unsigned char *sym_type) {
...
ElfW(Addr) ElfLookup(struct elf_img *restrict img, const char *restrict name, uint32_t hash, unsigned char *sym_type) {
...
ElfW(Addr) LinearLookup(struct elf_img *img, const char *restrict name, unsigned char *sym_type) {
...
ElfW(Addr) LinearLookupByPrefix(struct elf_img *img, const char *prefix, unsigned char *sym_type) {
  if (!_load_symtabs(img)) {
...
*/
static ElfW(Addr) handle_indirect_symbol(struct elf_img *img, ElfW(Off) offset) {
